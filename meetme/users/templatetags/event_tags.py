...
class EventCalendar(HTMLCalendar):
    ...
    def __init__(self, events):
        ...
        self.eventsbymonth = self.group_by_month(events)
        self.eventsbyday = self.group_by_day(events)
        ...
        if day != 0:
            ...
            if day in self.eventsbyday:
                ...
                """for event in self.events[day]:
                    body.append('<li>')
                    body.append('%s' % event.meeting_name)
                    body.append('</li>')"""
                return self.day_cell(cssclass, '<span style="color:red" class="dayNumber">%d</span> %s' % (day, ''.join(body)))
            return self.day_cell(cssclass, '<span class="dayNumberNoEvents">%d</span>' % (day))
        return self.day_cell('noday', '&nbsp;')

    def formatmonth(self, year, month):
        self.year, self.month = year, month
        if month not in self.eventsbymonth:
            return ""
    ...
    def group_by_month(self, events):
        monthField = lambda event: event.start_time.month
        if monthField is not None:
            print monthField
            return dict(
                [(month, list(items)) for month, items in groupby(events, monthField)]
            )
